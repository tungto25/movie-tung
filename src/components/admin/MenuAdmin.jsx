    ...
    return (
        <div className='p-5 bg-gray-950  md:h-screen'>
            <div className='flex justify-center items-center gap-5 ms-auto'>
                <RiMenuUnfold3Line onClick={(e) => setOpen(!open)} className={`text-2xl text-white ${open ? "" : "rotate-90"} transition-transform duration-200 hover:scale-120`} />
                {open && (
                    <div className='flex'>
            ...
            <div className={`${open ? "block" : "hidden"} md:block`}>
                <Link to={"/"} className={`flex gap-3 items-center mt-4 rounded-xl p-2 hover:text-yellow-500  bg-gray-200 hover:bg-gray-600 hover:text-white ${currentPath === "/" ? "text-yellow-500 bg-gray-500 shadow-md shadow-gray-200" : ""}`}>
                ...
                {menus.map((e, i) => (
                    <div>
                        ...
                        <ul className={dropbox == e.id ? "" : "hidden"}>
                            {e.items.map((b, i) => (
                                <Link className={`rounded-2xl block  mt-2 p-2 hover:text-yellow-500 border-b-1 ${currentPath === `/${b.path}` ? "text-yellow-500 bg-gray-500 shadow-md shadow-gray-200" : "bg-white"}`} to={`/${b.path}`} >{b.title}</Link >
                            ))}
                        </ul>
                    </div>
                ))}
                {open && (<h1 className='mt-4  text-white'>pages</h1>)}
                <Link to={"/user-pages"} className={`flex gap-3 items-center mt-4 rounded-xl p-2 hover:text-yellow-500  bg-gray-200 hover:bg-gray-600 hover:text-white ${currentPath === "/user-pages" ? "text-yellow-500 bg-gray-500 shadow-md shadow-gray-200" : ""}`}>
